...
\title{\textbf{Reporte Técnico de Diseño: Amplificador de Instrumentación de Ultra Precisión (SuperOpAmp)}}
...
El amplificador de instrumentación está fundamentado en la clásica topología de tres operacionales, pero llevada al límite físico reemplazando los operacionales individuales por macro-bloques compuestos de alta velocidad que llamamos \textbf{SuperOpAmp}.

\subsection{El Macro-Bloque SuperOpAmp}

La topología base del SuperOpAmp utiliza cinco amplificadores operacionales discretos combinados para crear etapas diferenciales con un control térmico y de error extremadamente superior.
...
\end{circuitikz}
\caption{Esquema vectorial de la topología interna del SuperOpAmp.}
\label{fig:superopamp}
\end{figure}

El diseño completo (`InstrAmpl.cir`) emplea dos de estos bloques `SuperOpAmp` como buffers de entrada (Fase 1) con ganancia cruzada, seguidos de un bloque final modificado para extraer la diferencia y excitar la carga pesada.
...
Utilizado en todas las etapas de ganancia y amplificación diferencial pura del \texttt{SuperOpAmp}.
\begin{itemize}
...
\newpage
\section{Conclusión}
La topología iterativa descrita en el presente informe de \textit{ngspice} cumple estrictamente el canon de rendimiento métrico esperado. La combinación paramétrica del \textit{ADA4817} y \textit{AD8397} encapsulados bajo el marco \textbf{SuperOpAmp} ha permitido alcanzar una ganancia global de **20,000**, una impedancia de salida residual a altas frecuencias de solo **$0.30\,\Omega$**, y una inmutabilidad asombrosa al modo común.
