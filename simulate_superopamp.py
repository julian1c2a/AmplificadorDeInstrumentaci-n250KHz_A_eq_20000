...
def run_simulation():
    cir_path = os.path.join(workspace_dir, "testbenches", "superopamp_ngspice.cir")
    print(f"Running ngspice simulation on: {cir_path}...")
    
    # Run ngspice in batch mode
    result = subprocess.run(
        [ngspice_path, "-b", os.path.join("testbenches", "superopamp_ngspice.cir")],
        cwd=workspace_dir,
        capture_output=True,
        text=True
    )
    ...
    log_path = os.path.join(txt_dir, "superopamp_sim_log.txt")
...
def analyze_and_plot():
    ...
    if len(dc_data) > 0:
        ...
        dc_df.to_csv(os.path.join(csv_dir, "superopamp_dc.csv"), index=False)
    else:
    ...
    if len(ac_data) > 0:
        ...
        ac_df.to_csv(os.path.join(csv_dir, "superopamp_ac.csv"), index=False)
    else:
    ...
    if len(tran_data) > 0:
        ...
        tran_df.to_csv(os.path.join(csv_dir, "superopamp_tran.csv"), index=False)
    else:
    ...
    fig_svg = os.path.join(svg_dir, "superopamp_analysis.svg")
